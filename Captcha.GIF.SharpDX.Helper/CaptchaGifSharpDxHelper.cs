 ...
 * https://www.cnblogs.com/sdflysha/p/dotnet-better-captcha.html
 * https://www.cnblogs.com/sdflysha/p/20191127-the-variant-bug-in-sharpdx.html
 * https://raw.githubusercontent.com/sdcb/blog-data/master/2019/20191127-the-variant-bug-in-sharpdx/WIC-Captcha-Gif-Ripple-Loop.linq
 */
namespace Microshaoft
{
    ...
    public static class CaptchaGifSharpDxHelper
    {

        public static unsafe byte[] SaveD2DBitmap(int width, int height, string text)
        {
            ...
            using var bmpLayer = new D2D.Bitmap1
                                            (
                                                dc
                                                , target.PixelSize
            ...
            D2D.Image oldTarget = dc.Target;
            {
                dc.Target = bmpLayer;
                dc.BeginDraw();
                var textFormat = new DWrite.TextFormat
                                        (
                                            dWriteFactory
                                            , "Times New Roman"
                ...
                for (int charIndex = 0; charIndex < text.Length; ++charIndex)
                {
                    ...
                    {
                new D2D.GradientStop{ Position = 0.0f, Color = GetColorFromHsl(r.NextFloat(0, 1), 1.0f, 0.8f) },
                new D2D.GradientStop{ Position = 1.0f, Color = GetColorFromHsl(r.NextFloat(0, 1), 1.0f, 0.8f) },
            }));
                ...
                for (var i = 0; i < 4; ++i)
                {
                    ...
                    brush.Color = GetColorFromHsl(r.NextFloat(0, 1), 1.0f, 0.3f);
                    target.DrawLine
                        (
            ...
            Color background = GetColorFromHsl(r.NextFloat(0, 1), 1.0f, 0.3f);
            var setMetadataMethod = encoder.MetadataQueryWriter
                .GetType()
                .GetMethod
                    (
                        nameof(WIC.MetadataQueryWriter.SetMetadataByName)
